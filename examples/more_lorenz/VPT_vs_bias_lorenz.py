...
RC_data         = np.zeros(biases.shape)
RC_data_std     = np.zeros(biases.shape)
NGRC_data       = np.zeros(biases.shape)
NGRC_data_std   = np.zeros(biases.shape)
hyb_data        = np.zeros(biases.shape)
hyb_data_std    = np.zeros(biases.shape)

for idx, bias in enumerate(tqdm(biases)):
    h = hybrid_rc_ngrc.Hyperparameters(bias=bias, prediction_steps=2000)
...
# since NGRC data does not depend on bias, use only the first value for plotting
...
plt.errorbar(biases, RC_data, yerr=RC_data_std/np.sqrt(trials), color=COLORS[1], marker='s', 
            markersize=4, ls=ls, linewidth=linewidth, label='RC')
plt.errorbar(biases, hyb_data, yerr=hyb_data_std/np.sqrt(trials), color=COLORS[3], marker='o', 
            markersize=4, ls=ls, linewidth=linewidth, label='Hybrid RC-NGRC')

plt.xlim((-2.25, 2.25))


plt.xlabel('Reservoir bias $c$')
...
plt.tight_layout()
plt.savefig('lorenz_VPT_vs_bias.png', dpi=300)